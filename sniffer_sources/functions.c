...
void capture_save () {
	FILE * capt;
	int i = 0;
	capt = fopen("/tmp/stats_sniffer", "wd+");
	if (capt < 0) {
		printf ("Can't save statistics!\n");
	}
	for (i = 0; i < 256; i++) {
		if ((hash_table[i].num) > 0) {
			fprintf(capt, "IP ADDRESS: %s\n", inet_ntoa(hash_table[i].ip_addr));
			fprintf(capt, "PACKET NUMS %lu\n", hash_table[i].num);
		}
	}
...
int init_conn () {
	...
	strcpy(ev_sock_struct.sun_path, "/tmp/ev_sock_file");

	unlink("/tmp/ev_sock_file");
